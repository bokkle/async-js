...
const loadNPause = async () => {
  try {
    ...
    console.error(`${err} :)`);
    imgContainer.insertAdjacentText('afterbegin', `${err}`);
    imgContainer.style.fontSize = '30px';
  }
};
loadNPause();
*/
...
const loadAll = async (imgArr) => {
  const imgs = Promise.all(imgArr.map((img) => createImage(img)))
  console.log(imgs)
};
...
loadAllIns(['img/img-1.jpg', 'img/img-2.jpg', 'img/img-3.jpg']);
